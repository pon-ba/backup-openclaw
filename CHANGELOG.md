...
- Infra/exec trust: keep blocked transparent-wrapper dispatches attributed to the actual blocked executable instead of the outer wrapper, so approval and audit flows report the right policy target. Thanks @vincentkoc.
- Install/update: allow package-manager installs from GitHub `main` via `openclaw update --tag main`, installer `--version main`, or direct npm/pnpm git specs. (#47630) Thanks @vincentkoc.
